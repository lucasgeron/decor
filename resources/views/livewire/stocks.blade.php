		...
		<thead>
			<tr>
				...
				<th class="w-2/12 text-center md:text-left md:w-4/12 lg:w-6/12" wire:click="sortBy('title')"
					style="cursor: pointer">Categorias @include('layouts.partials._sort-icon',['field'=>'title'])
				</th>
			...
			<tr class=" h-12 align-middle hover:bg-gray-100">
				...
				<td class="text-center "> {{-- status --}}
					<form method="post" wire:submit.prevent="toogle( {{ $category->id }} )">
						@if ($category->status)
						<button type="submit"
							class="px-3 rounded-lg bg-green-100 border border-green-300 text-green-500 ">ON</button>
						@else
						<button type="submit" href="#"
							class="px-2 rounded-lg bg-red-100 border border-red-300 text-red-500 ">OFF</button>
						@endif
					</form>
				</td>
				...
				<td class="text-center"> {{-- ammount --}}
					...
					<button wire:click="show('options', {{ $category->id }})"
				...
				<td class="text-center hidden sm:table-cell"> {{-- actions --}}
					...
					<div class="flex justify-between gap-1">
						...
						<button wire:click="delete({{ $category->id }})" class="flex-1 focus:outline-none
						rounded-lg bg-gray-100 border border-gray-300 text-gray-500 hover:bg-red-500
						hover:text-white
						hover:border-red-500 text-xs font-bold uppercase tracking-wide p-1">Confirmar</button>
					</div>
					@elseif($el===$category->id && $edit)
					<div class="flex justify-between gap-1">
						<button wire:click="hide('edit')"
							class="flex-1 focus:outline-none rounded-lg bg-gray-100 border border-gray-300 text-gray-500 hover:bg-gray-500 hover:text-white hover:border-gray-500 text-xs font-bold uppercase tracking-wide p-1">Cancelar</button>
						<button wire:click="update()"
							class="flex-1 focus:outline-none rounded-lg bg-gray-100 border border-gray-300 text-gray-500 hover:bg-indigo-700 hover:text-white hover:border-indigo-700 text-xs font-bold uppercase tracking-wide p-1">Confirmar</button>

					</div>
					@else
					<div class="flex justify-between gap-1"> --}}
						<button wire:click="show('delete', {{ $category->id }})"
							class=" text-center p-1 rounded-lg bg-gray-100 border hover:border-red-700 flex-1 uppercase tracking-wide text-gray-500  hover:text-red-700 text-xs font-bold align-middle focus:outline-none ">Remover</button>
						{{-- <button wire:click="show('edit', {{ $category->id }})"
							class=" text-center p-1 rounded-lg bg-gray-100 border hover:border-indigo-700 flex-1 uppercase tracking-wide text-gray-500  hover:text-indigo-700 text-xs font-bold align-middle focus:outline-none">Editar</button> --}}
							
							<button wire:click="editCategory({{ $category }})"
								class=" text-center p-1 rounded-lg bg-gray-100 border hover:border-indigo-700 flex-1 uppercase tracking-wide text-gray-500  hover:text-indigo-700 text-xs font-bold align-middle focus:outline-none">Editar</button>
						<a href="#"
							class="  text-center p-1 rounded-lg bg-gray-100 border hover:border-indigo-700 flex-1 uppercase tracking-wide text-gray-500  hover:text-indigo-700 text-xs font-bold align-middle focus:outline-none ">Ver</a>
					</div>
					{{-- @endif --}}
				</td>
...
<x-jet-dialog-modal wire:model="edit">
	...
	<x-slot name="content">
		...
		<div class="flex flex-wrap -mx-3">
			<div class="w-full px-3">
				<label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="titler">
					Título Passando Categoria inteira
				</label>
...
{{-- DELETE MODAL  --}}
<x-jet-dialog-modal wire:model="delete">
	<x-slot name="title">
		Remover Categoria
	</x-slot>
	...
	<x-slot name="content">
		...
		<div class="flex flex-wrap -mx-3">
			<div class="w-full px-3">
				...
				<input wire:model="update" disabled
					class="appearance-none block w-full bg-red-100 text-gray-700 border border-gray-200 rounded p-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
					type="text" placeholder="Categoria">
			</div>
		</div>

		<div class="flex justify-end  w-full py-2 ">
			<button wire:click="hide('delete')"
				class="flex-1 max-w-xs bg-gray-100 hover:bg-gray-500 focus:shadow-outline focus:outline-none text-gray-700 hover:text-white p-2 mr-2 rounded"
				type="button">
				Cancelar
			</button>

			<button wire:click="delete({{ $fwid }})"
...
{{-- OPTIONS FOR MOBILE  --}}
<x-jet-dialog-modal wire:model="options">
	<x-slot name="title">
		Ações
	</x-slot>
	...
	<x-slot name="content">
		...
		<div class=" justify-end gap-y-2 w-full py-2 ">
			<button wire:click="show('delete', {{ $fwid }})"
				class="mt-2 w-full  text-center p-3 rounded-lg bg-gray-100 border hover:border-red-700 uppercase tracking-wide text-gray-500  hover:text-red-700 text-xs font-bold align-middle focus:outline-none ">Remover</button>
			<button wire:click="show('edit', {{ $fwid }})"
				class="mt-2 w-full  text-center p-3 rounded-lg bg-gray-100 border hover:border-indigo-700 uppercase tracking-wide text-gray-500  hover:text-indigo-700 text-xs font-bold align-middle focus:outline-none ">Editar</button>
			{{-- <button  class="mt-2 w-full  text-center p-3 rounded-lg bg-gray-100 border hover:border-indigo-700 uppercase tracking-wide text-gray-500  hover:text-indigo-700 text-xs font-bold align-middle focus:outline-none "  >Ver</button> --}}
			<button wire:click="hide('options')"
				class="mt-2 w-full bg-gray-100 hover:bg-gray-500 focus:shadow-outline focus:outline-none text-gray-700 hover:text-white p-3 mr-2 rounded"
